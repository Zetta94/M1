...
function cacheFunction(cb) {
  ...
  Tu tarea aquí es lograr, mediante un closure, que cacheFunction actúe como una memoria caché para el callback
   que recibe por parámetro (cb); es decir, que "recuerde" el resultado de cada operación que hace, 
   de manera que, al realizar una operación por segunda vez, se pueda obtener el resultado de esa 
  ...
  argumento junto con el resultado de la invocación (tip: usá un objeto donde cada propiedad sea el argumento,
   y su valor el resultado de la correspondiente invocación a cb) de manera que, la próxima vez que reciba 
   el mismo argumento, no sea necesario volver a invocar a cb, porque el resultado estará guardado en la 
   "memoria caché".
 ...
 var resultados = {};
 return function(arg){
  if (!resultados.hasOwnProperty(arg.toString())){
    resultados[arg.toString()] = cb(arg);
  }
...
let getNombreInstructor = getNombre.bind(instructor);//!Siempre tiene que ir asociado a un objeto!!!
